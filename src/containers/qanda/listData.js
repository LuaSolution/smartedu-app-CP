...
import { Row, Card, CustomInput, Badge } from 'reactstrap'
                  ...
                  <div className="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                    <NavLink to={`?p=${item.id}`} className="w-80 w-sm-100">
                      <p className="list-item-heading mb-2 truncate">
                        <b>Tiêu đề: </b><Tooltip title={item.question_title} color={'#2db7f5'}>
                          {item.question_title}
                        </Tooltip>
                        <br />
                        Khóa học: <b> {item.title}</b>
                      </p>
                    </NavLink>
                    {item.status === 0 ? <Badge color={'danger'} pill style={{ fontSize: 10 }}>
                      Chưa trả lời
                    </Badge> :
