...
import logo from 'atoms/course-contents/logo.svg'
import { Skeleton, Button, Tooltip } from 'antd'
import { connect } from 'react-redux'
import { initFinishedLectures, addLectureToFinishedCourses, updateCourseContents } from 'redux/actions'
...
const HeaderSection = ({ courseTitle, lectureTitle, courseId, lectureType, timeToSkip, questionsToSkipBak, questionsToSkip, finishedLectures, initFinishedLectures, updateCourseContents }) => {
    ...
    return courseTitle ?
        <CourseContentHeader>
            <Navbar bg="" expand="lg">
                ...
                <Navbar.Collapse id="basic-navbar-nav">
                    ...
                    <Nav className="ifa-mr-auto">
                        <Nav.Link href="#" className="ifa-process-link" style={{ marginRight: 32 }}>
                            <div>
                                {btnDoTest && lectureType !== 4 && <Button onClick={openDoTestModal}>{'Câu hỏi qua bài'}</Button>}
                                {!btnDoTest && <><div id="clock" className="box" style={{ display: 'none' }}>
            ...
            <TrueAnswerModal modal={toggleModal} closeModal={() => {
                setToggleModal(false)
                setBtnDoTest(false)
            }} />
            <WrongAnswerModal modal={toggleFailedModal} closeModal={() => { setToggleFailedModal(false) }} />
        </CourseContentHeader >
        :
        suspense
}
...
const mapStateToProps = ({ courseContents }) => {
    const { courseId, courseTitle, lectureTitle, timeToSkip, questionsToSkip, lectureType, allowToNext, questionsToSkipBak, finishedLectures } = courseContents
    return { courseId, allowToNext, courseTitle, lectureTitle, lectureType, timeToSkip, questionsToSkip, questionsToSkipBak, allowToNext, finishedLectures }
}
