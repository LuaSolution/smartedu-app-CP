...
import AddNewModal from 'containers/news/addAndEdit'
import News from 'containers/news/listData'
...
const orderOptions = [
  { column: 'title', label: 'Tiêu đề bài viết' },
...
const NewsPage = ({ match, ...props }) => {
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/managers/news?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/managers/news')
    }
