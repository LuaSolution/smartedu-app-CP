...
const header02 = ['STT', 'Họ tên', 'Tên đăng nhập', 'Email', 'Ngày sinh', 'Ngày bắt đầu học', 'Trạng thái hoàn thành', 'Ngày hoàn thành', 'Kết quả', 'Phần trăm hoàn thành']
const metadataWeek = () => { return ['', 'Tuần', '', '', '', '', '', '', '', '', ''] }
const metadata1 = value => { return ['', 'Tên khóa học', value, '', '', '', '', '', '', '', ''] }
const metadata2 = value => { return ['', 'nhóm người dùng (khu vực)', value, '', '', '', '', '', '', '', ''] }
      ...
      axios.post('admin/analytics/export-report/1', params)
        .then(res => {
          if (res.data.status === 200) {
            ...
            const row2 = metadata3(selectData.departments[selectData.departments.findIndex(i => i.id === department.value)].name)
        ...
        <Card>
          <CardBody>
            ...
            >
              ...
              <Form.Item label="nhóm người dùng (khu vực)">
                <Select
                  showSearch
                  optionFilterProp="children"
                  filterOption={(input, option) =>
