...
import AddNewModal from 'containers/mentorCalls/addAndEdit'
import UserList from 'containers/mentorCalls/listData'
...
const UsersPages = ({ match, ...props }) => {
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      axios
        ...
        .then((data) => {
          if (data.status === 200) {
            setTotalPage(Math.ceil(data.data.totalItem / selectedPageSize))
            setItems(data.data.data)
            setSelectedItems([])
            setTotalItemCount(data.data.totalItem)
          }
        })
  ...
  const removeSelected = () => {
    ...
    axios
      .post('admin/mentor-calls/delete', {
        selected: _selectedItems,
      })
      .then((res) => {
        const newItems = items.filter(
          (i) => !selectedItems.some((j) => i.id === j)
        )
        setItems(newItems)
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/users/mentor-calls/list?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/users/mentor-calls')
    }
  ...
  const editUser = (item) => {
    ...
    var foundIndex = newItems.findIndex(
