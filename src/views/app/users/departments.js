...
import AddNewModal from 'containers/departments/addAndEdit'
import ListData from 'containers/departments/listData'
...
const orderOptions = [
  { column: 'name-asc', label: 'Tên bộ phận A-Z' },
  { column: 'name-desc', label: 'Tên bộ phận Z-A' },
  { column: 'id-desc', label: 'Ngày tạo mới nhất' },
  { column: 'id-asc', label: 'Ngày tạo cũ nhất' },
]
const pageSizes = [4, 8, 12, 20]

const DepartmentsPage = ({ match, ...props }) => {
  ...
  const onContextMenuClick = (e, data) => {
    ...
    if (data.action === 'delete') {
      ...
      props.history.push('/app/users/departments?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/users/departments')
    }
