...
import AddNewModal from 'containers/permissions/addAndEdit'
import ListData from 'containers/permissions/listData'
...
const orderOptions = [
  { column: 'name-asc', label: 'Tên chức vụ A-Z' },
  { column: 'name-desc', label: 'Tên chức vụ Z-A' },
  { column: 'id-desc', label: 'Ngày tạo mới nhất' },
  { column: 'id-asc', label: 'Ngày tạo cũ nhất' },
]
const pageSizes = [4, 8, 12, 20]

const PermissionsPage = ({ match, ...props }) => {
  ...
  const onContextMenuClick = (e, data) => {
    ...
    if (data.action === 'delete') {
      ...
      props.history.push('/app/users/permissions?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/users/permissions')
    }
