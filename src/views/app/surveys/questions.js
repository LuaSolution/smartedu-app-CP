...
import AddNewModal from 'containers/surveyQuestions/addAndEdit'
import SurveyQuestions from 'containers/surveyQuestions/listData'
...
const SurveyQuestionsPage = ({ match, deleteUsers, ...props }) => {
  ...
  const removeSurveyQuestions = () => {
    axios
      .post('admin/surveys/questions/delete', {
        selectedItems
      })
      .then((res) => {
        setItems(items.filter(item => !selectedItems.includes(item.id)))
        successMessage('Xóa câu hỏi khảo sát thành công')
      })
      .catch((error) => {
        errorMessage('Xóa câu hỏi khảo sát thất bại')
      })
  }
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/surveys/questions/?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/surveys/questions')
    }
  ...
  const editSurveyQuestions = item => {
  ...
  return <Spin spinning={loading}>
    <div className="disable-text-selection">
      <ListPageHeading
        ...
        removeSelected={removeSurveyQuestions}
      ...
      <AddNewModal
        ...
        addNew={appendNew}
        editSurveyQuestions={editSurveyQuestions}
      />
      <SurveyQuestions
        items={items}
