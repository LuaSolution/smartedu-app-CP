...
import AddNewModal from 'containers/surveys/addAndEdit'
import Surveys from 'containers/surveys/listData'
...
const orderOptions = [
  { column: 'title-desc', label: 'Tên khảo sát (A-Z)' },
  { column: 'title-asc', label: 'Tên khảo sát (Z - A)' },
...
const SurveyPage = ({ match, ...props }) => {
  ...
  const removeSurveyList = () => {

    axios
      .post('admin/surveys/delete', {
        selectedItems
      })
      .then(res => {
        setItems(items.filter(item => !selectedItems.includes(item.id)))
        message.success('Xóa bảng khảo sát khảo sát thành công')
      })
      .catch(error => {
        message.error('Xóa bảng khảo sát khảo sát thất bại')
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/surveys?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/surveys/default')
    }
  ...
  return <Spin spinning={loading}>
    <div className="disable-text-selection">
      <ListPageHeading
        ...
        removeSelected={removeSurveyList}
