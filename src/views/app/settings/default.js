...
import AddNewModal from 'containers/systemNotifications/addAndEdit'
import ListData from 'containers/systemNotifications/listData'
...
const orderOptions = [
  { column: 'title', label: 'Tiêu đề thông báo' },
  { column: 'id-desc', label: 'Ngày tạo mới nhất' },
  { column: 'id-asc', label: 'Ngày tạo cũ nhất' },
]
const pageSizes = [4, 8, 12, 20]

const SystemNotificationsPage = ({ match, deleteUsers, loading, ...props }) => {
  ...
  useEffect(() => {
    const fetchData = () => {
      setLoading(true)
      axios
        .get('admin/systemNotifications/paging', {
          params: {
  ...
  const removeData = () => {
    axios
      .post('admin/systemNotifications/delete', {
        selectedItems
      })
      .then(res => {
      ...
      .catch(error => {
        errorMessage('Xóa thông báo thất bại')
      })
  }
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/settings/default?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/settings/default')
    }
