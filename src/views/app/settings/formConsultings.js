...
import AddNewModal from 'containers/formConsultings/addAndEdit'
import ListData from 'containers/formConsultings/listData'
...
const orderOptions = [
  // { column: 'question_title-desc', label: 'Tiêu đề (Z-A)' },
  // { column: 'question_title-asc', label: 'Tiêu đề (A-Z)' },
  { column: 'id-desc', label: 'Ngày tạo mới nhất' },
  { column: 'id-asc', label: 'Ngày tạo cũ nhất' },
]
const pageSizes = [4, 8, 12, 20]

const QandaPage = ({ match, deleteUsers, loading, ...props }) => {
  ...
  useEffect(() => {
    const fetchData = () => {
      setLoading(true)
      axios
        .get('admin/form-consultings/paging', {
          params: {
  ...
  const removeqanda = () => {
    axios
      .post('admin/form-consultings/delete', {
        selectedItems
      })
      .then(res => {
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/settings/form-consultings?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/settings/form-consultings')
    }
  ...
  return <Spin spinning={isLoading}>
    <div className="disable-text-selection">
      <ListPageHeading
        hideAddMore={true}
        hideViewType={true}
        heading="settings.form-consultings"
