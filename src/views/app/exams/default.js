...
import AddNewModal from 'containers/exams/addAndEdit'
import ListData from 'containers/exams/listData'
...
const orderOptions = [
  { column: 'title', label: 'Tiêu đề đề thi' },
  { column: 'id-desc', label: 'Ngày tạo mới nhất' },
  { column: 'id-asc', label: 'Ngày tạo cũ nhất' },
]
const pageSizes = [4, 8, 12, 20]

const QuestionsPage = ({ match, ...props }) => {
  const [currentPage, setCurrentPage] = useState(1)
  const [selectedPageSize, setSelectedPageSize] = useState(8)
  const [selectedOrderOption, setSelectedOrderOption] = useState({ column: 'id-desc', label: 'Ngày tạo' })
  ...
  useEffect(() => { setCurrentPage(1) }, [selectedPageSize, selectedOrderOption])

  useEffect(() => {
    const fetchData = () => {
      setLoading(true)
      axios
        .get('admin/exams/paging', {
          params: {
        ...
        .then(data => {
          if (data.status === 200) {
            setTotalPage(Math.ceil(data.total / selectedPageSize))
            setItems(data.data)
            setSelectedItems([])
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/exams/default?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/exams/default')
    }
