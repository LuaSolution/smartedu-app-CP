...
import AddNewModal from 'containers/questions/addAndEdit'
import ListData from 'containers/questions/listData'
...
const orderOptions = [
  { column: 'content', label: 'Tiêu đề câu hỏi' },
...
const QuestionsPage = ({ match, ...props }) => {
  ...
  const onContextMenuClick = (e, data) => {
    if (data.action === 'delete') {
      ...
      props.history.push('/app/exams/questions?p=' + selectedItems[0])
    }
  }
  ...
  const toggleModal = () => {
    if (modalOpen) {
      props.history.push('/app/exams/questions')
    }
