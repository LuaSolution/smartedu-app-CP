...
const NewsPage = props => {
  ...
  const fetchData = () => {
    setLoading(true)
    axios.get('tin-tuc/paging/' + Math.ceil(data.length / pageSize))
      .then(res => {
  ...
  const renderDetail = item => {
    return <Card>
      <CardBody>
        ...
        <h3 style={{ textAlign: 'center', fontWeight: 'bold', margin: '50px 0' }}>{item.title}</h3>
  ...
  return <>
    ...
    <div className="ifa-body-wrapper">
      <div className="ifa-container ifa-news-elearning" style={{ backgroundImage: `url(${bgNewsElearning})`, marginTop: 0, paddingTop: 50 }}>
        <div className="container ifa-container ifa-news-block">
          <div className="ifa-block-content container">
            <p className="top-small-text">
              Liên tục những hoạt động sôi nổi
            </p>
            ...
            <Spin spinning={loading}>
              <div className="row list-news">
                {data && data.map((item, index) =>
                  <div className="col-lg-4 col-md-12" key={index} style={{ marginBottom: 15 }}>
                    <div className="item ifa-item" style={{ border: '1px solid #eee' }}>
                      <a href={'/news?p=' + item.id}>
                        ...
                        <div className="content">
                          <div className="status-wrapper">
                            ...
                            <span className="status comming">{momentTime(item.created_at)}</span>
                          </div>
                          <div className="title">
            ...
            {!loading && <div className='btn-load-more-expert' style={{ marginTop: 40 }}>
